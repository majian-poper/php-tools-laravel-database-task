...
use PHPTools\LaravelDatabaseTask\Contracts\BatchableDatabaseTaskInterface;
...
class CompleteDatabaseTask implements ShouldQueue
{
    use Queueable;
    ...
    {
        $taskInstance = $this->task->toTask();

        if (! ($taskInstance instanceof BatchableDatabaseTaskInterface)) {
            return;
        }
        ...
        try {
            DB::transaction(fn() => $this->runTask($this->task, $taskInstance));
        } catch (\Throwable $e) {
            $this->runTaskFailed($this->task, $e);
        }
    }

    protected function runTask(DatabaseTask $task, BatchableDatabaseTaskInterface $taskInstance): void
    {
        $batches = $task->batches()
            ->with('output')
            ->where('status', TaskStatus::PROCESSED)
            ->orderBy('batch_index')
            ->get();

        $outputs = $batches->map(fn($batch) => $batch->output->toOutput())->all();

        $output = $taskInstance->mergeOutputs($outputs);
